...
var makeSolRequestMessage = function (core, _a) {
    ...
    return new apiUtils.OperationResolver(commonAuthUtils.requestChallengeSolanaOperation, BASE_URL, core).fetch(__assign({ network: commonSolUtils.SolNetwork.create(solNetwork).network, address: commonSolUtils.SolAddress.create(address).toString() }, options));
};
var makeAptosRequestMessage = function (core, _a) {
    ...
    return new apiUtils.OperationResolver(commonAuthUtils.requestChallengeAptosOperation, BASE_URL, core).fetch(__assign({ chainId: commonAptosUtils.AptosNetwork.create(chain).network, address: commonAptosUtils.AptosAddress.create(address).toString() }, options));
};
