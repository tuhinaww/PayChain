...
function deserializeResponse$a(_a) {
    ...
    return __assign(__assign({}, jsonResponse), { chain: commonAptosUtils.AptosNetwork.create(chainId), address: commonAptosUtils.AptosAddress.create(jsonResponse.address), expirationTime: commonCore.maybe(jsonResponse.expirationTime, function (value) { return new Date(value); }), notBefore: commonCore.maybe(jsonResponse.notBefore, function (value) { return new Date(value); }) });
}
...
function getRequestBody$8(request) {
    return {
        ...
        chainId: commonAptosUtils.AptosNetwork.create(request.chainId).toString(),
...
function serializeRequest$9(request, core) {
    return {
        domain: request.domain,
        chainId: commonAptosUtils.AptosNetworkResolver.resolve(request.chainId, core),
        address: commonAptosUtils.AptosAddress.create(request.address).toString(),
        publicKey: request.publicKey,
        statement: request.statement,
        uri: request.uri,
...
function deserializeRequest$9(jsonRequest) {
    return {
        ...
        chainId: commonAptosUtils.AptosNetwork.create(jsonRequest.chainId),
